...
export class CartService implements ICartService {
  ...
  public deleteCart(id: string): Observable<boolean> {
    return this.http.delete<DeleteCartResponse>(`${BACKEND_API}/carts/items/${id}`)
    .pipe(
  ...
  public addProductToCart(product: { item_id: number, quantity: number }): Observable<boolean> {
    return this.http.post<AddProductToCartResponse>(`${BACKEND_API}/carts/add-product`, undefined, { params: { ...product } })
    .pipe(
      catchError((err: HttpErrorResponse) => {
        console.error(err);
        return of(err.message);
  ...
  public getAllCarts(): Observable<Cart[]> {
    return this.http.get<GetAllCartsResponse>(`${BACKEND_API}/carts`)
    .pipe(
      catchError((err: HttpErrorResponse) => {
        console.error(err);
        return of(err.message);
  ...
  public decreaseItemQuantity(id: number): Observable<boolean> {
    return this.http.patch<DecreaseItemQuantityResponse>(`${BACKEND_API}/items/${id}/decrease"`, undefined)
    .pipe(
      catchError((err: HttpErrorResponse) => {
        console.error(err);
        return of(err.message);
